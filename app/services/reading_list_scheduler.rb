class ReadingListScheduler
  ...
  MIN_DAILY_SHARE = 10    # minutes — don't flatten a book below this per day
  ...
  # Score: [bucket, projected]
  #
  # Prefer combinations that get above target by the smallest amount.
  # Bucket 0 = at/above target (prefer lowest projected — smallest overshoot).
  # Bucket 1 = below target (prefer highest projected — closest to target).
  def evaluate_combination(combo, current_load, target, current_best)
    ...
    if projected >= target
      score = [0, projected]   # above: smallest overshoot wins
    else
      score = [1, -projected]  # below: closest to target wins
    end
